...
def generate_auto_reflection_auto_adapt_prompt(question, previous_incorrect_full_responses, auto_prompt_model_config_key, main_model_name, main_model_api_key, main_model_api_provider, benchmark_name, config_data):
    """
    Generates an adaptive self-reflection prompt.
    The prompt itself is generated by a model (either the main model or a specified one).
    """
    
    model_to_use_for_meta_prompt: str
    api_key_to_use_for_meta_prompt: str
    api_provider_to_use_for_meta_prompt: str
    supports_sampling_for_meta_prompt = True # Default, can be overridden

    if auto_prompt_model_config_key == "same":
        model_to_use_for_meta_prompt = main_model_name
        api_key_to_use_for_meta_prompt = main_model_api_key
        api_provider_to_use_for_meta_prompt = main_model_api_provider
        # Find the main model in config to get its supports_sampling_params
        for model_key, model_details_val in config_data.get("models", {}).items():
            if model_details_val.get("name") == main_model_name:
                supports_sampling_for_meta_prompt = model_details_val.get("supports_sampling_params", True)
                break
    else:
        # auto_prompt_model_config_key is a key like "gpt-4o", "llama-3.1-70b-instruct" etc.
        specific_model_details = config_data.get("models", {}).get(auto_prompt_model_config_key)
        if not specific_model_details:
            print(f"[Error] Auto-prompt model key \'{auto_prompt_model_config_key}\' not found in config.json models. Falling back to main model.")
            model_to_use_for_meta_prompt = main_model_name
            api_key_to_use_for_meta_prompt = main_model_api_key
            api_provider_to_use_for_meta_prompt = main_model_api_provider
            for model_key, model_details_val in config_data.get("models", {}).items():
            ...
            model_to_use_for_meta_prompt = specific_model_details["name"]
            api_provider_to_use_for_meta_prompt = specific_model_details["provider"]
            api_key_env_var = specific_model_details.get("api_key_env")
            supports_sampling_for_meta_prompt = specific_model_details.get("supports_sampling_params", True)
            
            if not api_key_env_var:
                print(f"[Error] 'api_key_env' not defined for auto-prompt model \'{auto_prompt_model_config_key}\'. Falling back to main model.")
                model_to_use_for_meta_prompt = main_model_name
                api_key_to_use_for_meta_prompt = main_model_api_key
                api_provider_to_use_for_meta_prompt = main_model_api_provider
                # Redundant lookup but safe
                for model_key, model_details_val in config_data.get("models", {}).items():
                    if model_details_val.get("name") == main_model_name:
                        supports_sampling_for_meta_prompt = model_details_val.get("supports_sampling_params", True)
                        break
                ...
                if not fetched_api_key:
                    print(f"[Error] API key not found via env var \'{api_key_env_var}\' for auto-prompt model \'{auto_prompt_model_config_key}\'. Falling back to main model.")
                    model_to_use_for_meta_prompt = main_model_name
                    api_key_to_use_for_meta_prompt = main_model_api_key
                    api_provider_to_use_for_meta_prompt = main_model_api_provider
                    for model_key, model_details_val in config_data.get("models", {}).items():
                    ...
                    api_key_to_use_for_meta_prompt = fetched_api_key

    if not api_key_to_use_for_meta_prompt: # Final check if API key is somehow still None
        print(f"[Error] API Key for meta-prompt generation is missing. Cannot generate adaptive prompt. Problem model: {model_to_use_for_meta_prompt}")
        return None # Cannot proceed

    current_meta_prompt = META_PROMPT_TEMPLATE.format(question=question)
    
    print(f"[Info] Generating adaptive reflection instructions using model: {model_to_use_for_meta_prompt} (Provider: {api_provider_to_use_for_meta_prompt})")
    adapted_reflection_prompt_instructions = query_model(
        api_key=api_key_to_use_for_meta_prompt,
        prompt=current_meta_prompt,
        model=model_to_use_for_meta_prompt,
        supports_sampling_params=supports_sampling_for_meta_prompt,
        api_provider=api_provider_to_use_for_meta_prompt
    )

    if adapted_reflection_prompt_instructions is None:
        print(f"[Warning] Failed to generate adaptive reflection instructions using {model_to_use_for_meta_prompt}.")
        return None # Or return a default traditional reflection prompt
...
Question: {question}
Your initial Chain-of-Thought answer (full response) was: {previous_incorrect_full_responses[0] if previous_incorrect_full_responses else 'None'}"""
    
    if len(previous_incorrect_full_responses) > 1:
        final_reflection_prompt_for_main_model += "\n\nYour previous reflection full responses were:"
        for i, full_resp in enumerate(previous_incorrect_full_responses[1:], 1):
            final_reflection_prompt_for_main_model += f"\nReflection Attempt {i} (full response): {full_resp}"
